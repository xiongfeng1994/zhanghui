...
public class SpringDataRedisCache implements DistributedCache {
	...
	public boolean setObject2Json(final String key, final Object value) {
		final String jsonObject = JSON.toJSONString(value);
		boolean result = redisTemplate.execute(new RedisCallback<Boolean>() {
			public Boolean doInRedis(RedisConnection conn) throws DataAccessException {
	...
	public Boolean setNx(final String key, final Object value) {
		return redisTemplate.opsForValue().setIfAbsent(key, value == null ? null : JSONObject.toJSONString(value));
	}

	@Override
	public Boolean setNx(final  String key,final Object value, final int expire) {
		Boolean rst = redisTemplate.opsForValue().setIfAbsent(key,value == null ? null : JSONObject.toJSONString(value));
